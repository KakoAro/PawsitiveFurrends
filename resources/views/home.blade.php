            ...
            <div class="row align-items-center g-5">
                <div class="col-lg-6">
                    <div class="hero-badge">
                        <span
                            style="width:6px;height:6px;background:var(--terra);border-radius:50%;display:inline-block"></span>
                    ...
                    <div class="d-flex gap-4 mt-4 pt-4" style="border-top:1px solid var(--tan)">
                        <div>
                            <div class="font-display fw-bold" style="font-size:2rem;color:var(--cocoa)">
                                {{ number_format($totalPets) }}+</div>
                ...
                <div class="col-lg-6 d-none d-lg-block">
                    <div class="position-relative" style="max-width:420px;margin:auto">
                        <div
                            style="border-radius:40% 60% 60% 40%/50% 40% 60% 50%;overflow:hidden;aspect-ratio:4/5;background:var(--terra-light)">
                        ...
                        {{-- Floating card --}}
                        <div class="card border-0 shadow-sm position-absolute"
                            style="top:10%;right:-5%;border-radius:0.9rem;padding:0.7rem 1rem;background:var(--card-bg)">
                            <div class="d-flex align-items-center gap-2" style="font-size:0.82rem">
                                ...
                                <div>
                                    ...
                                    <span
                                        style="font-size:0.72rem;background:var(--sage-light);color:var(--sage);padding:2px 8px;border-radius:50px">✓
                                        Just Adopted!</span>
                                </div>
                            </div>
                        </div>
                        <div class="card border-0 shadow-sm position-absolute"
                            style="bottom:12%;left:-5%;border-radius:0.9rem;padding:0.7rem 1rem;background:var(--card-bg)">
    ...
    <section class="py-5" id="pets">
        <div class="container">
            ...
            <div class="d-flex gap-2 flex-wrap justify-content-center mb-4">
                ...
                <a href="{{ route('pets.index') }}?species=dog"
                    class="filter-tab {{ request('species') === 'dog' ? 'active' : '' }}">🐶 Dogs</a>
                <a href="{{ route('pets.index') }}?species=cat"
                    class="filter-tab {{ request('species') === 'cat' ? 'active' : '' }}">🐱 Cats</a>
                            ...
                            <div class="p-3">
                                <div class="d-flex justify-content-between align-items-center mb-1">
                                    ...
                                    <span class="badge rounded-pill"
                                        style="background:var(--cream);color:var(--muted);font-weight:400;font-size:0.75rem">{{ $pet->age_string }}</span>
                                </div>
                                <div class="mb-2" style="font-size:0.82rem;color:var(--muted)">{{ $pet->breed }} ·
                                    {{ ucfirst($pet->size) }}</div>
                                ...
                                <div class="d-flex justify-content-between align-items-center">
                                    ...
                                    @else
                                        <a href="{{ route('pets.show', $pet) }}" class="btn btn-terra btn-sm px-3"
                                            onclick="event.stopPropagation()">Adopt</a>
                                    @endif
                                </div>
                            </div>
                        </div>
    ...
    <section class="py-5 bg-warm-white" id="how">
        <div class="container">
            <div class="text-center mb-5 fade-up">
                ...
                <p style="color:var(--muted);font-weight:300;max-width:480px;margin:auto">We've made it easy to find, meet,
                    and bring home your new best friend in just a few steps.</p>
            </div>
            <div class="row g-4 text-center">
                @foreach ([['icon' => '🔍', 'title' => 'Browse & Search', 'desc' => 'Filter by species, age, size, and breed to find pets that match your lifestyle perfectly.', 'bg' => 'var(--terra-light)'], ['icon' => '💬', 'title' => 'Connect with Shelter', 'desc' => 'Reach out to the shelter directly to ask questions and arrange a meet-and-greet visit.', 'bg' => 'var(--sage-light)'], ['icon' => '📋', 'title' => 'Apply to Adopt', 'desc' => 'Fill out a simple adoption application. Our team reviews it within 24 hours.', 'bg' => 'var(--terra-light)'], ['icon' => '🏠', 'title' => 'Welcome Home!', 'desc' => 'Once approved, bring your new companion home and start building a beautiful bond.', 'bg' => 'var(--sage-light)']] as $i => $step)
    ...
    <section class="py-5" id="categories">
        <div class="container">
            ...
            <div class="row g-3 fade-up">
                @foreach ([['emoji' => '🐶', 'name' => 'Dogs', 'species' => 'dog'], ['emoji' => '🐱', 'name' => 'Cats', 'species' => 'cat']] as $cat)
                    <div class="col-6 col-sm-4 col-lg">
                        <a href="{{ route('pets.index') }}?species={{ $cat['species'] }}"
                            class="cat-card {{ $cat['species'] === 'dog' ? 'featured' : '' }}">
                            <div class="cat-emoji">{{ $cat['emoji'] }}</div>
                            <div class="cat-name">{{ $cat['name'] }}</div>
                            <div class="cat-count">{{ number_format($speciesCounts[$cat['species']] ?? 0) }} available
                            </div>
                        </a>
                    </div>
                @endforeach
            ...
            <div class="cta-banner p-5">
                <div class="row align-items-center g-4">
                    <div class="col-lg-7" style="position:relative">
                        <h2 class="font-display text-white mb-2">Ready to find your <em
                                style="color:var(--terra-light)">forever friend?</em></h2>
                        <p style="color:var(--cocoa-light);font-weight:300;max-width:440px">
                            Join thousands of families who've found their perfect companion through PawHome. Every adoption
                            saves a life.
                        </p>
                    </div>
                    <div class="col-lg-5 text-lg-end d-flex gap-3 justify-content-lg-end flex-wrap"
                        style="position:relative">
                        <a href="{{ route('pets.index') }}" class="btn btn-white px-4 py-2">Browse Pets 🐾</a>
                        @guest
                            <a href="{{ route('register') }}" class="btn btn-ghost px-4 py-2"
                                style="color:var(--cocoa-light);border:1.5px solid rgba(255,255,255,0.2);border-radius:50px">Sign
                                Up Free</a>
                        @endguest
                    </div>
                </div>
