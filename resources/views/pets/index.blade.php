            ...
            <div class="row g-4">
                ...
                <div class="col-lg-3">
                    <div class="card border-0 shadow-sm"
                        style="border-radius:1.2rem;background:var(--card-bg);position:sticky;top:90px">
                                    ...
                                    @foreach (['dog' => '🐶 Dogs', 'cat' => '🐱 Cats'] as $val => $label)
                                        <div class="form-check mb-1">
                                            <input class="form-check-input" type="radio" name="species"
                                                value="{{ $val }}" id="sp_{{ $val }}"
                                                {{ request('species') === $val ? 'checked' : '' }}
                                                onchange="this.form.submit()">
                                            <label class="form-check-label" for="sp_{{ $val }}"
                                                style="font-size:0.88rem">{{ $label }}</label>
                                        </div>
                                    @endforeach
                                    @if (request('species'))
                                        <a href="{{ route('pets.index') }}"
                                            style="font-size:0.78rem;color:var(--terra)">Clear</a>
                                    @endif
                                </div>
                                    ...
                                    @foreach (['small' => 'Small', 'medium' => 'Medium', 'large' => 'Large', 'extra_large' => 'Extra Large'] as $val => $label)
                                        <div class="form-check mb-1">
                                            <input class="form-check-input" type="radio" name="size"
                                                value="{{ $val }}" id="sz_{{ $val }}"
                                                {{ request('size') === $val ? 'checked' : '' }}
                                                onchange="this.form.submit()">
                                            <label class="form-check-label" for="sz_{{ $val }}"
                                                style="font-size:0.88rem">{{ $label }}</label>
                                        </div>
                                    @endforeach
                                </div>
                                    ...
                                    @foreach (['male' => 'Male', 'female' => 'Female'] as $val => $label)
                                        <div class="form-check mb-1">
                                            <input class="form-check-input" type="radio" name="gender"
                                                value="{{ $val }}" id="gd_{{ $val }}"
                                                {{ request('gender') === $val ? 'checked' : '' }}
                                                onchange="this.form.submit()">
                                            <label class="form-check-label" for="gd_{{ $val }}"
                                                style="font-size:0.88rem">{{ $label }}</label>
                                        </div>
                                    @endforeach
                                </div>
                                    ...
                                    @foreach (['baby' => 'Baby (0–1 yr)', 'young' => 'Young (1–3 yrs)', 'adult' => 'Adult (3–8 yrs)', 'senior' => 'Senior (8+ yrs)'] as $val => $label)
                                        <div class="form-check mb-1">
                                            <input class="form-check-input" type="radio" name="age_group"
                                                value="{{ $val }}" id="ag_{{ $val }}"
                                                {{ request('age_group') === $val ? 'checked' : '' }}
                                                onchange="this.form.submit()">
                                            <label class="form-check-label" for="ag_{{ $val }}"
                                                style="font-size:0.88rem">{{ $label }}</label>
                                        </div>
                                    @endforeach
                                </div>
                        ...
                        <div class="d-flex gap-2 align-items-center">
                            ...
                            <form action="{{ route('pets.index') }}" method="GET" class="d-flex">
                                ...
                                <div class="input-group input-group-sm">
                                    <input type="text" name="search" class="form-control"
                                        placeholder="Search name or breed" value="{{ request('search') }}"
                                        style="border-radius:50px 0 0 50px">
                                    <button class="btn btn-terra btn-sm" style="border-radius:0 50px 50px 0"><i
                            ...
                            <form action="{{ route('pets.index') }}" method="GET">
                                ...
                                <select name="sort" class="form-select form-select-sm" onchange="this.form.submit()"
                                    style="border-radius:50px;font-size:0.82rem;min-width:120px">
                    ...
                    <div class="row g-4">
                                    ...
                                    <div class="p-3">
                                        <div class="d-flex justify-content-between align-items-center mb-1">
                                            ...
                                            <span class="badge rounded-pill"
                                                style="background:var(--cream);color:var(--muted);font-weight:400;font-size:0.72rem">{{ $pet->age_string }}</span>
                                        </div>
                                        <div class="mb-2" style="font-size:0.82rem;color:var(--muted)">
                                            {{ $pet->breed ?? 'Mixed' }} · {{ ucfirst($pet->size) }}</div>
                        ...
                        @empty
                            <div class="col-12 text-center py-5">
                                ...
                                <p style="color:var(--muted)">Try adjusting your filters or <a
                                        href="{{ route('pets.index') }}">clear all filters</a>.</p>
                            </div>
                        @endforelse
                    </div>
